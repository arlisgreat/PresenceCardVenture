...
export async function buildApp(options: { uploadsDir?: string; store?: DemoStore; uploadDailyLimit?: number } = {}): Promise<FastifyInstance> {
  ...
  app.post('/v1/device/ack', async (r:any, reply) => { const token=String(r.headers.authorization??'').replace(/^Bearer\s+/i,''); if(!store.userForToken(token)&&!([...store.devices.values()].some(d=>d.token===token)))return reply.code(401).send(errorBody('TOKEN_INVALID','token invalid')); return reply.code(204).send() })
