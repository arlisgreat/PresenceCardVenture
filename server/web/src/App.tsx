...
function App() {
  ...
  const [inviteCodeFromUrl] = useState(() => new URLSearchParams(window.location.search).get('join')?.replace(/\D/g, '').slice(0, 6) ?? '')
...
function MessagesView({ onToast, initialCode = '' }: { onToast: (message: string) => void; initialCode?: string }) {
  ...
  useEffect(() => { void getCurrentUser().then(user => { setInviteCode(user.friend_code); setInviteUrl(`${window.location.origin}/?join=${encodeURIComponent(user.friend_code)}`) }).catch(() => { setInviteUrl(`${window.location.origin}/?join=${encodeURIComponent(inviteCode)}`) }) }, [])
