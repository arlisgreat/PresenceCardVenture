...
import { AiJob, FeedItem, Message, PlayType, createAiJob, deletePhoto, getAiJob, getDeviceState, getFeed, getMessages, reactToPhoto, sendMessage, uploadPhoto } from './api'
import './styles.css'
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-column">
        ...
        {view === 'feed' && <FeedView feed={visibleFeed} circle={circle} onCircle={setCircle} onReact={onReact} onDelete={onDelete} onCreate={() => setView('create')} />}
...
function FeedView({ feed, circle, onCircle, onReact, onDelete, onCreate }: { feed: FeedItem[]; circle: string; onCircle: (value: string) => void; onReact: (item: FeedItem) => void; onDelete: (item: FeedItem) => void; onCreate: () => void }) {
  return <section className="content-wrap feed-view">
    ...
    <div className="feed-grid">{feed.length === 0 ? <div className="empty-state"><span>◌</span><h2>圈子还在等第一张照片</h2><p>释放今天的一个瞬间，朋友会在这里遇见它。</p></div> : feed.map(item => <PhotoCard key={item.id} item={item} onReact={() => onReact(item)} onDelete={() => onDelete(item)} />)}</div>
...
function PhotoCard({ item, onReact, onDelete }: { item: FeedItem; onReact: () => void; onDelete: () => void }) {
  const liked = (item.my_reactions ?? []).includes('heart')
  return <article className="photo-card">
    <div className="photo-frame"><img src={item.image_url} alt={item.caption ?? '朋友分享的照片'} /><span className="photo-type">{item.filter_id === 'film' ? 'CCD' : item.filter_id === 'template' ? 'TEMPLATE' : 'PHOTO'}</span></div>
    <div className="photo-meta"><div className="meta-line"><div className="mini-avatar">{item.author.display_name.slice(0, 1)}</div><div><b>{item.author.display_name}</b><span>{formatTime(item.created_at)} · {item.circle ?? '小圈'}</span></div><button className="more-button" aria-label="更多操作">···</button></div><p className="caption">{item.caption ?? '今天也好好在场。'}</p><div className="reaction-line"><button className={liked ? 'reaction active' : 'reaction'} onClick={onReact}><span>✦</span>{item.reactions.heart ?? 0}</button><button className="reaction" onClick={onReact}><span>⌁</span>拍一拍</button>{item.mine && <button className="delete-link" onClick={onDelete}>移除</button>}</div></div>
  </article>
}
...
function CreateView({ onPublished, onCancel }: { onPublished: (item: FeedItem) => void; onCancel: () => void }) {
  ...
  return <section className="content-wrap studio-view"><div className="studio-head"><div><p className="section-kicker">CREATE / 释放</p><h2>把今天折成一张小卡。</h2><p>选择一个房间，云端会替你把它修成想要的样子。</p></div><button className="quiet-button" onClick={onCancel}>返回圈子</button></div>
    ...
    <div className="studio-layout"><div className="upload-stage" onClick={() => inputRef.current?.click()}>{preview ? <img className={`preview-image filter-${filterId}`} src={preview} alt="待处理预览" /> : <div className="upload-prompt"><span>＋</span><strong>放一张照片进来</strong><small>JPG / PNG · 最大 1MB</small></div>}<input ref={inputRef} type="file" accept="image/jpeg,image/png" onChange={event => selectFile(event.target.files?.[0])} /><div className="stage-stamp">{filterId === 'none' ? 'ORIGINAL' : filterId.toUpperCase()}</div></div>
...
function MessagesView({ onToast }: { onToast: (message: string) => void }) {
  const [friend, setFriend] = useState('luna'); const [messages, setMessages] = useState<Message[]>([]); const [draft, setDraft] = useState('')
  ...
  return <section className="content-wrap messages-view"><div className="studio-head"><div><p className="section-kicker">SIGNALS / 轻回应</p><h2>一句话，轻轻抵达。</h2><p>没有未读红点，只有朋友留在这里的信号。</p></div><span className="signal-count">3 条新信号</span></div><div className="messages-layout"><div className="friend-list">{[['luna', '露娜', '今天 18:42'], ['momo', '墨墨', '昨天 21:06'], ['jiji', '吉吉', '周三 09:12']].map(([id, name, time]) => <button key={id} className={friend === id ? 'friend-row active' : 'friend-row'} onClick={() => setFriend(id)}><span className="friend-avatar">{name.slice(0, 1)}</span><span><b>{name}</b><small>{time}</small></span><i>·</i></button>)}</div><div className="conversation"><div className="conversation-head"><span>与 <b>{friend === 'luna' ? '露娜' : friend === 'momo' ? '墨墨' : '吉吉'}</b> 的小信号</span><span className="online-label">● 在场</span></div><div className="message-stream">{messages.map(message => <div key={message.id} className={message.sender === 'me' ? 'bubble mine' : 'bubble'}><p>{message.body}</p><small>{formatTime(message.createdAt)}</small></div>)}</div><div className="message-compose"><button aria-label="添加图片">＋</button><input value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void send() }} placeholder="写一句话……" /><button className="send-button" onClick={() => void send()}>→</button></div></div></div></section>
}
...
function AiView({ feed, onToast }: { feed: FeedItem[]; onToast: (message: string) => void }) {
  ...
  return <section className="content-wrap ai-view"><div className="studio-head"><div><p className="section-kicker">AI STUDIO / 合照</p><h2>让两份在场，遇见一次。</h2><p>选择已授权的素材，云端会为你们合成一张新的记忆。</p></div><div className="consent-badge">⌁ 素材已授权</div></div><div className="ai-layout"><div className="material-grid">{candidates.map(item => <button key={item.id} className={selected.includes(item.id) ? 'material selected' : 'material'} onClick={() => setSelected(current => current.includes(item.id) ? current.filter(id => id !== item.id) : current.length < 2 ? [...current, item.id] : current)}><img src={item.image_url} alt="可用于合照的照片" /><span>{selected.includes(item.id) ? `0${selected.indexOf(item.id) + 1}` : '＋'}</span><small>{item.author.display_name} · {item.filter_id}</small></button>)}</div><div className="ai-result"><div className="result-art">{job?.status === 'completed' ? <img src={job.resultUrl} alt="AI 合照结果" /> : <><span className="result-symbol">⌁</span><p>{job?.status === 'processing' ? '正在把两份记忆放在一起…' : job?.status === 'queued' ? '已排队，云端马上开始…' : '合照会出现在这里'}</p></>}</div><div className="job-status"><div><span className={job?.status === 'completed' ? 'status-dot done' : 'status-dot'} /><b>{job?.status === 'completed' ? '合照已生成' : job ? '云端处理中' : '等待选择素材'}</b></div><button className="primary-button" onClick={() => void create()} disabled={job?.status === 'queued' || job?.status === 'processing'}>{job?.status === 'completed' ? '再生成一张' : '生成合照'}<span>→</span></button></div></div></div></section>
}
...
function DeviceView({ state, feed, onToast }: { state: { unseen_count: number; pending_friend_requests: number; server_time: string }; feed: FeedItem[]; onToast: (message: string) => void }) {
  ...
  return <section className="content-wrap device-view"><div className="studio-head"><div><p className="section-kicker">DEVICE LAB / 联调</p><h2>小卡，准备好在场。</h2><p>用模拟器验证上传、拉取、下发和轻回应，不需要真实硬件。</p></div><span className="device-pill"><span className="live-dot" /> dvc_a1b2c3d4</span></div><div className="device-layout"><div className="device-card"><div className="device-screen"><div className="screen-top"><span>小卡 · 01</span><span>Wi-Fi ●</span></div><div className="screen-photo">{feed[0] ? <img src={feed[0].image_url} alt="设备当前照片" /> : <span>等待照片</span>}</div><div className="screen-bottom"><span>✦ {feed[0]?.reactions.heart ?? 0}</span><span>320 × 240</span></div></div><div className="device-controls"><button onClick={() => log('POST /v1/photos · 201 · photo uploaded')}>拍照并上传 <span>↑</span></button><button onClick={() => log(`GET /v1/feed · 200 · ${feed.length} items`)}>拉取圈子 <span>↓</span></button><button onClick={() => log('POST /v1/device/ack · 204 · acknowledged')}>回执轻信号 <span>✦</span></button></div></div><div className="device-info"><div className="info-row"><span>设备状态</span><b className="green-text">在线</b></div><div className="info-row"><span>未看动态</span><b>{state.unseen_count}</b></div><div className="info-row"><span>待处理好友</span><b>{state.pending_friend_requests}</b></div><div className="info-row"><span>最后同步</span><b>{state.server_time ? formatTime(state.server_time) : '刚刚'}</b></div><div className="log-box"><p>REQUEST LOG</p>{logs.map((log, index) => <code key={`${log}-${index}`}>{log}</code>)}</div></div></div></section>
}
