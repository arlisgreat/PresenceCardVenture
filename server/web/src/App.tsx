...
import { AiJob, FeedItem, Friend, FriendRequest, Message, PlayType, acceptFriendRequest, bindDevice, createAiJob, deleteAiJob, deletePhoto, deviceAck, deviceHeartbeat, getAiJob, getDeviceState, getFeed, getFriendRequests, getFriends, getMessages, getPairStatus, pokePhoto, reactToPhoto, requestPairCode, sendFriendRequest, sendMessage, uploadPhoto } from './api'
import './styles.css'
...
function MessagesView({ onToast }: { onToast: (message: string) => void }) {
  ...
  const [friends, setFriends] = useState<Friend[]>([{ username: 'luna', display_name: '露娜' }, { username: 'momo', display_name: '墨墨' }]); const [requests, setRequests] = useState<FriendRequest[]>([]); const [friendCode, setFriendCode] = useState(''); const [showAddFriend, setShowAddFriend] = useState(false); const [friendBusy, setFriendBusy] = useState(false)
