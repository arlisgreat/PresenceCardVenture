...
- 设备配对持久化切片已完成 `PrismaDeviceStore` 的安全边界测试（短期配对码、过期校验、device token hash）。暂未接入 `/pair/*` 路由：当前协议要求绑定后由 `/pair/status` 再次返回明文 token，而生产 schema 只保存 hash；接入前需将绑定响应改为一次性返回 token 或增加受保护的加密凭证字段，避免设备无法完成登录。
